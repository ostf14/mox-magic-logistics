...
/** Цветом класса красится только обводка: и у ромба, и у тултипа. */
  ...
  return (
    <span className="group relative inline-flex items-center">
      <button
        ...
        className={`inline-flex h-[1.125rem] w-[1.125rem] rotate-45 items-center justify-center border bg-paper focus:outline-none focus-visible:ring-1 focus-visible:ring-ink ${TONE[hazardClass]}`}
      >
        <span className="-rotate-45 font-mono text-[0.5625rem] font-medium leading-none tracking-tight text-muted">
          {ROMAN[hazardClass]}
        </span>
      </button>
      <span
        className={`pointer-events-none absolute bottom-[calc(100%+0.5rem)] right-0 z-10 w-60 border bg-card p-3 text-xs leading-relaxed text-ink opacity-0 transition-opacity group-hover:opacity-100 group-focus-within:opacity-100 ${TONE[hazardClass]}`}
        role="tooltip"
      >
